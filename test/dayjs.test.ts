...
describe("DayJS", () => {
	...
	test("Clamp", () => {
		...
		// Tests the cases
		const clampBefore = dayjs.clamp(clampBeforeDate, clampMin, clampMax);
		const clampBetween = dayjs.clamp(clampBetweenDate, clampMin, clampMax);
		const clampAfter = dayjs.clamp(clampAfterDate, clampMin, clampMax);
